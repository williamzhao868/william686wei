...
const getLocalHrPeers = () => {
  ...
  for (const article of fallbackArticles.filter((item) => item.type === 'A')) {
    ...
    const hasHrCompany = (merged.companies || []).some((company) => HR_COMPANIES.has(company.en));
    if (hasHrCompany) {
  ...
  for (const article of getLocalInsightArticles()) {
    const hasHrCompany = (article.companies || []).some((company) => HR_COMPANIES.has(company.en));
    if (!hasHrCompany) continue;
...
function HrPeersPage() {
  ...
  const fetchArticles = async () => {
    ...
    try {
      ...
      const localItems = getLocalHrPeers();
      const localByTitle = new Map(localItems.map((item) => [String(item.title || '').trim(), item]));
      const mergedLiveItems = (records.items || [])
        .map((item) => mergeWithLocalContent(
          item,
  ...
  const filterableArticles = useMemo(() => articles.map(enrichContentForFilters), [articles]);
  const keywordsData = useMemo(() => buildKeywordCloudData(filterableArticles), [filterableArticles]);
